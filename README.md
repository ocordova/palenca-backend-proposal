...
For better developer experience, maintainability and testing I propose this repos
strategy:
...
This will run all the services with network connection between
containers
...
mock                     # Mock API Project
   |-- <sub_app>         # Sub apps following Clean Arch. (see section below)
       |-- domain        # Entities, enums, adapters, usecases
           |-- tests
       |-- presentation  # Resources, response models and validations
           |-- tests
...
project, examples are Client, User, Platform, AppLogin, Earnings, Profile.
An entity is typically saved to the database using a repository method
(see below), but they can also be retrieved from APIs using HTTP.
...
webhooks and raises exceptions. The use cases typically call repository methods.
...
`presentation/validations` Endpoint payload validation are build using the pydantic
